...
import com.jyuka.dto.response.ArticleClientResponse;
...
//    @Disabled("실제 API 호출 결과 관찰용이므로 평상시엔 비활성화")
    ...
    @EnableConfigurationProperties(ProjectProperties.class) // ConfigurationProperties 를 사용하여 slice test 시 불러오지를 못함, 이에 직접 불러와줌
    @AutoConfigureWebClient(registerRestTemplate = true) // BeanConfig 에서 RestTemplate 을 가져온다.
    @RestClientTest(ArticleManagementService.class) // RestClient 를 mock 처리를 위함, 테스트할 대상 : ArticleManagementService
    @Nested // 중첩 테스트를 가능하게 한다.
    class RestTemplateTest {

        private final ArticleManagementService sut;
        private final ProjectProperties projectProperties;
        private final MockRestServiceServer server; // 실질적으로 mocking 이 이루어짐
        ...
        void givenNothing_whenCallingArticlesApi_thenReturnsArticleList() throws Exception {
            ...
            // RestTemplate이 호출시 응답값에 대한 mocking 처리
            ...
            ;
            server.verify(); // 요청이 실제로 이루어졌는지 테스트
        }
